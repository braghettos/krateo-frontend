...
export interface PortalActionProposal {
  ...
  /** publishBlueprint (FE-BP6) / publishPage (FE-BP7), MUTATING: the STEP-A git publish for a
   * blueprint chart / a portal page. The model emits ONLY these repo-coordinate scalars; the HOST
   * fans them out into the gitrefs + per-file repocontents + pullrequests op set from the HELD
   * previewed draft (the model stalls hand-writing that multi-op payload). All optional — each
   * defaults to the blueprint-catalog (publishBlueprint) or portal-chart (publishPage) repo.
   * `title`/`body`/`namespace` above are shared; these add the rest. */
  owner?: string
  repo?: string
  base?: string
