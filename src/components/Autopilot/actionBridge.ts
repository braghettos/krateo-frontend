...
export const PORTAL_BUILDER_ROUTING_DIRECTIVE = [
  ...
  'PREVIEW SELF-CORRECTION: if the PAGE CONTEXT envelope carries `previewProblems`, the LAST previewed page was REJECTED by validation — those lines are the EXACT schema errors, one per failing field. Do NOT wait to be asked and do NOT apologize-and-stop: your reply MUST fix exactly those errors in the affected CRs and re-emit the FULL corrected {"verb":"previewPage",...} fence (every widget + RESTAction). If widget-CR authoring is the frontend specialist\'s domain here, re-delegate to it with the `previewProblems` lines verbatim and forward its corrected CR set in the fence.',
  '</portal_builder_routing>',
].join('\n')
