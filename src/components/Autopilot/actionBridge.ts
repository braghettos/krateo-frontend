...
export const PORTAL_CAPABILITIES_PROMPT = [
  ...
  'PREVIEWS — four READ-ONLY preview verbs render into the portal\'s preview drawer (auto-applied, nothing is written): {"verb":"previewBlueprint","chart":{"url":"<chart url>","version":"<version>"},"values":{...},"label":"..."} helm-renders a blueprint chart as a dry run; {"verb":"previewPage","widgets":[<widget CR objects>],"label":"..."} shows proposed widget CRs as source; {"verb":"previewRestDef","restDefinition":{<full RestDefinition CR draft>},"label":"..."} shows a KOG API-mapping draft — its YAML, its mapped `action · METHOD path` lines, client-side validation errors, and the immutability warnings; {"verb":"explainUpgradeImpact","namespace":"<composition ns>","name":"<composition name>","toVersion":"<target version>","label":"..."} renders what a blueprint version Update would change (added / removed / modified objects + whether the values schema changed) — emit it BEFORE proposing the Update control for an outdated composition. ALWAYS preview before proposing the write that creates those objects; the draft is shown in the PREVIEW DRAWER, never as YAML in your prose.',
