...
export interface PortalActionProposal {
  ...
  /** publishBlueprint (FE-BP6, MUTATING): STEP-A blueprint git publish. The model emits ONLY
   * these repo-coordinate scalars; the HOST fans them out into the gitrefs + per-file
   * repocontents + pullrequests op set from the HELD previewed tree (the model stalls
   * hand-writing that multi-op payload). All optional — each defaults to the blueprint-catalog
   * repo. `title`/`body`/`namespace` above are shared; these add the rest. */
  owner?: string
  repo?: string
  base?: string
...
export const PORTAL_CAPABILITIES_PROMPT = [
  ...
  'PORTAL BUILDER — AUTHOR A NEW PORTAL PAGE FROM A CONVERSATION: when the user wants to BUILD a new portal PAGE / dashboard / view (not a blueprint, not an API mapping), you draft the page as widget CRs and publish it via git to the portal chart. A page is a set of widget CRs in namespace krateo-system: a ROOT Flex named `page-<slug>` (metadata.name MUST be `page-<slug>` — this is the route `/<slug>` and the page identity) that lists its child widgets, the child widgets themselves (Card/Table/Listy/Flex/Statistic/… — kinds from the widget registry, apiVersion widgets.templates.krateo.io/v1beta1), and any RESTAction CRs the widgets read (apiVersion templates.krateo.io/v1). Wire children by `resourceRefId` ↔ `resourcesRefs.items[].id`; derive EVERY value server-side (jq in a RESTAction / widgetDataTemplate) — never client-compute or fabricate data. WORKFLOW: (1) PREVIEW — emit {"verb":"previewPage","widgets":[<FULL widget CR objects, each with kind + metadata.name>],"nav":{"label":"<sidebar label>","icon":"fa-<icon>","order":950},"label":"..."}. The OPTIONAL `nav` sets the page\'s sidebar entry (label / FontAwesome icon / order; every field defaults — label from the slug, icon fa-file, order 950 so authored pages sort after the built-in nav). The portal validates each CR against its widget schema and renders the source + validation verdicts in the preview drawer; iterate with a NEW previewPage after every correction. A publish is DENIED unless the SAME page (its `page-<slug>` root) was previewed in this thread. (2) PUBLISH — emit ONE applyResourceSet writing github.krateo.io CRs to braghettos/krateo-portal-chart, IN ORDER: POST gitrefs (payload spec {"configurationRef":{"name":"github-blueprints-config"},"owner":"braghettos","repo":"krateo-portal-chart","ref":"refs/heads/builder/page-<slug>","sha":"<krateo-portal-chart main HEAD sha from page context>"}); then one POST repocontents PER widget-CR file where content is EXACTLY the token {"$fileContent":"<kind-lower>.<name>.yaml"} — NEVER the bytes — and path is chart/templates/<kind-lower>.<name>.yaml (payload spec {"configurationRef":{"name":"github-blueprints-config"},"owner":"braghettos","repo":"krateo-portal-chart","path":"chart/templates/<kind-lower>.<name>.yaml","branch":"builder/page-<slug>","message":"<msg>","content":{"$fileContent":"<kind-lower>.<name>.yaml"}}); then ONE MORE POST repocontents for the page\'s NAV FRAGMENT — path chart/files/nav-fragments/<slug>.yaml and content EXACTLY the token {"$fileContent":"nav-fragment.<slug>.yaml"} (the portal auto-generated this fragment from the page\'s page-<slug> root at preview time and holds it; menu.sidebar-nav.yaml globs files/nav-fragments/*.yaml, so this ships the /<slug> sidebar entry WITH the page — NO manual menu edit) (payload spec {"configurationRef":{"name":"github-blueprints-config"},"owner":"braghettos","repo":"krateo-portal-chart","path":"chart/files/nav-fragments/<slug>.yaml","branch":"builder/page-<slug>","message":"<msg>","content":{"$fileContent":"nav-fragment.<slug>.yaml"}}); then POST pullrequests (payload spec {"configurationRef":{"name":"github-blueprints-config"},"owner":"braghettos","repo":"krateo-portal-chart","head":"builder/page-<slug>","base":"main","title":"builder: page <slug>","body":"<summary — the page AND its /<slug> sidebar entry both ship in this PR>"}). Cap 10 ops (1 GitRef + widget files + 1 nav fragment + 1 PR); a page with >6 widget files splits across turns on the SAME branch (GitRef once, then RepoContent chunks incl. the nav fragment, then the PR). The `$fileContent` slug MUST exactly equal `<kind-lower>.<name>.yaml` of a widget you previewed — the portal HOLDS the previewed CRs and substitutes the verbatim YAML (base64) at publish time; never inline, retype, or summarize widget bytes. Widget CRs reach the cluster ONLY through this git write — NEVER emit a direct applyResourceSet op on widgets.templates.krateo.io or restactions (DENIED). (3) GO-LIVE — after the human merges the PR and CI tags + publishes the portal OCI chart, propose the 1-op gated PATCH of compositiondefinitions.core.krateo.io/portal spec.chart.version to the new tag; the CDC re-renders and snowplow serves the new page (no restart).',
