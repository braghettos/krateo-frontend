...
export const PORTAL_CAPABILITIES_PROMPT = [
  ...
  'REMEDIATE VIA AN ON-PAGE CONTROL WHEN ONE FITS: separately from the missing-operator case above, when a failing composition can be fixed by a control ALREADY on this page, proactively propose that ONE runAction instead of only describing the problem — e.g. a Paused composition -> its Resume control; a stuck or last-failed reconcile -> its Sync / force-reconcile control; an out-of-date blueprint version -> its Update control. The user confirms it in a blast-radius dialog that shows exactly what will change (verb, resource, namespace/cluster, and a diff), so proposing it is safe — the human still approves before anything runs. Prefer the LEAST-disruptive control; only propose an action that actually exists in the page context; and NEVER propose Delete as a \'fix\' unless the user explicitly asked to remove the composition.',
